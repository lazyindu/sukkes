...
async def pub_(bot, message):
    ...
    if locked:
        try:
          ...
          async for message in iter_messages(client, chat_id=sts.get("FROM"), limit=sts.get("limit"), offset=sts.get("firstmsg"), filters=filter, max_size=max_size):
...
#     i = sts.get(full=True)
#     status_text = (
#         'Forwarding' if status == 5 
#         else f"Sleeping {status} s" if str(status).isnumeric() 
#         else status
#     )
    
#     # Calculate the current percentage
#     percentage = int(float(i.fetched) * 100 / float(i.total))
    
#     # Use an attribute on sts to track the last update percentage to avoid excessive edits
#     if not hasattr(sts, 'last_update'):
#         sts.last_update = 0

#     # Update only if we've increased progress by at least 10%
#     # Always update when reaching 100% to show completion
#     if percentage < 100 and percentage < sts.last_update + 10:
#         return  # Skip update to avoid frequent editing
#     sts.last_update = percentage  # Update our tracking value
    
#     # Create a simple progress bar with 10 steps (each step equals 10%)
#     progress = "●" * (percentage // 10) + "○" * (10 - (percentage // 10))
    
#     # Build the status message text (only showing percentage, progress bar, and current status)
#     text = f"**Progress:** {percentage}%\n{progress}\n\n**Status:** {status_text}"
    
#     # Construct inline buttons for progress status and cancel/complete actions
#     button = [[InlineKeyboardButton(f"Progress: {percentage}%", f'fwrdstatus#{status}#{percentage}#{i.id}')]]
#     if status in ["cancelled", "completed"]:
#         button.append([InlineKeyboardButton("✅ Completed", url="https://t.me/ComanderAK")])
#     else:
#         button.append([InlineKeyboardButton("❌ Cancel", "terminate_frwd")])
    
#     # Safely update the message; msg_edit handles any FloodWait errors gracefully
...
async def store_vars(user_id):
    ...
    print(fetch)
    STS(id=forward_id).store(settings['chat_id'], settings['toid'], settings['skip'], settings['firstmsg'], settings['limit'])
    return forward_id
